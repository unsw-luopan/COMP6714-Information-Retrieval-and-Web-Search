...
class DataReader(object):
    def __init__(self, config, file_name, input_word_dict, input_char_dict, output_tag_dict, batch_size,
                 is_train=False):
        self.config = config
    ...
    def read_data(self, file_name):
        ...
        char_index_matrices = [[[self.get_char_ids(char) for char in word] for word in sentence] for sentence in
                               sentence_list]
    ...
    def __next__(self):
        ...
        [input_word_index_lists, input_char_index_matrices, input_tag_index_lists] = list(
            zip(*self.batch_list[self.current_batch_index]))
        ...
        batch_char_index_matrices = np.zeros(
            (len(input_word_index_lists), max(batch_sentence_len_list), max(map(max, input_word_len_lists))), dtype=int)
        batch_char_mask = np.zeros(
            (len(input_word_index_lists), max(batch_sentence_len_list), max(map(max, input_word_len_lists))), dtype=int)
        for i, (input_char_index_matrix, word_len_list) in enumerate(
                zip(input_char_index_matrices, input_word_len_lists)):
        ...
        batch_word_len_lists = np.ones((len(input_word_index_lists), max(batch_sentence_len_list)),
